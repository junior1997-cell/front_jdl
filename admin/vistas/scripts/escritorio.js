...
function tablero() {   
  ...
  $.post("../ajax/escritorio.php?op=tablero",  function (e, status) {
    ...
    if (e.status == true) {
      $("#cantidad_box_trabajdor").html(`${e.data.cant_trabajador}`);
      $("#cantidad_box_compra").html(`S/. ${formato_miles(e.data.total_compra)}`);
      $("#cantidad_box_venta").html(`S/. ${formato_miles(e.data.total_venta)}`);
