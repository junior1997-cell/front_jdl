        ...
        if ($(".producto_" + idproducto).hasClass("producto_selecionado")) {          
          ...
          <tr class="filas" id="fila${cont}">         
            ...
            <td class="py-1">         
              ...
              <input type="hidden" name="ficha_tecnica[]" value="${e.data.ficha_tecnica}">
              <div class="user-block text-nowrap w-100">
                ...
                <span class="username"><p class="mb-0 "> <textarea name="nombre_producto[]" cols="1" rows="1" class="nombre_producto_${cont} w-100" >${e.data.nombre_producto}</textarea> </p></span>
            ...
            <td class="py-1 form-group"><input class="w-135px precio_venta_${cont} form-control" type="number" name="precio_venta[]" value="${(e.data.precio_venta)}" min="0.01" required onkeyup="modificarSubtotales();" onchange="modificarSubtotales();"></td>
            ...
            <td class="py-1 text-right"><span class="subtotal_producto_${cont}" >0</span> <input type="hidden" class="inp_subtotal_producto_${cont}" name="subtotal_producto[]" value="${(e.data.subtotal_producto)}" ></td>
...
function evaluar() {
  if (detalles > 0) {
    $("#guardar_registro_cotizacion").show();
  } else {
    $("#guardar_registro_cotizacion").hide();
    cont = 0;
    $(".subtotal_cotizacion").html("S/ 0.00");
    $("#subtotal_cotizacion").val(0);

    $(".descuento_cotizacion").html("S/ 0.00");
    $("#descuento_cotizacion").val(0);

    $(".igv_cotizacion").html("S/ 0.00");
    $("#igv_cotizacion").val(0);

    $(".total_cotizacion").html("S/ 0.00");
    $("#total_cotizacion").val(0);

  }
}

function default_val_igv() { if ($("#precio_con_igv").select2("val") == "Si") { $("#impuesto").val(0.18); } }

function modificarSubtotales() {  

  var val_igv = $('#impuesto').val(); //console.log(array_venta);

  if ($("#precio_con_igv").select2("val") == null) {
    ...
    if (array_venta.length === 0) {
      ...
      array_venta.forEach((element, index) => {
        ...
        var precio_venta = parseFloat($(`.precio_venta_${element.id_cont}`).val());
        var descuento = parseFloat($(`.descuento_${element.id_cont}`).val());
        var subtotal_producto = 0;

        // Calculamos: IGV
        var precio_sin_igv = precio_venta;
        ...
        subtotal_producto = cantidad * parseFloat(precio_venta) - descuento;
        $(`.subtotal_producto_${element.id_cont}`).html(formato_miles(subtotal_producto));
        $(`.inp_subtotal_producto_${element.id_cont}`).val(redondearExp(subtotal_producto, 2));        
      });
      calcularTotalesSinIgv();
    }  
  } else if ($("#precio_con_igv").select2("val") == "Si") {    
    ...
    if (array_venta.length === 0) {
      ...
      array_venta.forEach((element, index) => {
        ...
        var precio_sin_igv = ( quitar_igv_del_precio(precio_venta, val_igv, 'decimal')).toFixed(2);
        $(`.precio_sin_igv_${element.id_cont}`).val(precio_sin_igv);

        // Calculamos: IGV
        var igv = (parseFloat(precio_venta) - parseFloat(precio_sin_igv)).toFixed(2);
        $(`.precio_igv_${element.id_cont}`).val(igv);
  ...
  } else if ($("#precio_con_igv").select2("val") == "No") { 
    ...
    if (array_venta.length === 0) {
      ...
      array_venta.forEach((element, index) => {
        ...
        // Calculamos: Subtotal de cada producto
        subtotal_producto = cantidad * parseFloat(precio_venta) - deacuento;
        $(`.subtotal_producto_${element.id_cont}`).html(formato_miles(subtotal_producto));
        $(`.inp_subtotal_producto_${element.id_cont}`).val(redondearExp(subtotal_producto, 2));
      });
...
function calcularTotalesSinIgv() {
  var total = 0.0; var descuento = 0;

  if (array_venta.length === 0) {
  } else {
    array_venta.forEach((key, index) => {
      total += parseFloat($(`.inp_subtotal_producto_${key.id_cont}`).val()); console.log( `${key.id_cont} total: ${total}`);
      descuento += parseFloat($(`.descuento_${key.id_cont}`).val()); console.log( `descuento: ${descuento}`);
    });

    $(".subtotal_cotizacion").html("S/ " + formato_miles(total));
    $("#subtotal_cotizacion").val(redondearExp(total, 2));

    $(".descuento_cotizacion").html("S/ " + formato_miles(descuento));
    $("#descuento_cotizacion").val(redondearExp(descuento, 2));

    $(".igv_cotizacion").html("S/ 0.00");
    $("#igv_cotizacion").val(0.0);
    $(".val_igv").html('IGV (0%)');

    $(".total_cotizacion").html("S/ " + formato_miles(total));
    $("#total_cotizacion").val(redondearExp(total, 2));
  }
}

function calcularTotalesConIgv() {
  var val_igv = $('#impuesto').val();
  var igv = 0; var descuento = 0; var total = 0; var subotal_sin_igv = 0;
  array_venta.forEach((key, index) => {
    total += parseFloat($(`.inp_subtotal_producto_${key.id_cont}`).val());  
    descuento += parseFloat($(`.descuento_${key.id_cont}`).val());
  });

  subotal_sin_igv = quitar_igv_del_precio(total, val_igv, 'decimal');
  igv = (parseFloat(total) - parseFloat(subotal_sin_igv));

  $(".subtotal_cotizacion").html(`S/ ${formato_miles(subotal_sin_igv)}`);
  $("#subtotal_cotizacion").val(redondearExp(subotal_sin_igv, 2));

  $(".descuento_cotizacion").html("S/ " + formato_miles(descuento));
  $("#descuento_cotizacion").val(redondearExp(descuento, 2));

  $(".igv_cotizacion").html("S/ " + formato_miles(igv));
  $("#igv_cotizacion").val(igv);
      ...
      if (e.data.detalle) {
        ...
        e.data.detalle.forEach((element, index) => {
          ...
          <tr class="filas" id="fila${cont}">
            ...
            <td class="form-group"><input type="number" class="w-135px precio_venta_${cont} form-control" type="number"  name="precio_venta[]" id="precio_venta[]" value="${parseFloat(element.precio_venta).toFixed(2)}" min="0.01" required onkeyup="modificarSubtotales();" onchange="modificarSubtotales();"></td>
